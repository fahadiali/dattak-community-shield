...
app = FastAPI(title="Site B - PME Demo")
...
async def home(request: Request):
    ...
    return templates.TemplateResponse("index.html", {
        "request": request,
        "site_name": "Site B - Expertise Comptable Martin"
    })
...
async def submit_contact(
    ...
    print(f"[SITE B] Legitimate form submission from {client_ip}")
    return JSONResponse({
        "status": "success",
...
if __name__ == "__main__":
    import uvicorn
    print(f"Starting {SITE_ID} on port 8002...")
    print(f"Home: http://localhost:8002")
